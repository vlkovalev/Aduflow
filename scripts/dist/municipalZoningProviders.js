...
exports.lookupMunicipalOpenData = lookupMunicipalOpenData;
...
async function lookupMunicipalOpenData(address) {
    const location = await geocodeAddress(address);
    if (!location)
        return null;
    const municipality = location.municipality.toLowerCase();
    if (municipality === "edmonton")
        return lookupEdmonton(location);
    if (municipality === "calgary")
        return lookupCalgary(location);
    if (municipality === "vancouver")
        return lookupVancouver(location);
    if (municipality === "surrey")
        return lookupSurrey(location);
    if (municipality === "courtenay")
        return lookupCourtenay(location);
    if (municipality === "toronto")
        return lookupToronto(location);
    if (municipality === "seattle")
        return lookupSeattle(location);
    if (municipality === "portland")
        return lookupPortland(location);
    return null;
}
async function geocodeAddress(address) {
    const cacheKey = (0, node_crypto_1.createHash)("sha256").update(address.trim().toLowerCase()).digest("hex");
    ...
    const records = await queuedGeocodeFetch(`https://nominatim.openstreetmap.org/search?${query}`);
    const record = records[0];
    if (!record) {
        rememberGeocode(cacheKey, null);
        return null;
    }
    const details = (record.address ?? {});
...
async function queuedGeocodeFetch(url) {
    let releaseQueue = () => undefined;
    const previous = geocodeQueue;
    geocodeQueue = new Promise((resolve) => {
...
async function lookupEdmonton(location) {
...
async function lookupCalgary(location) {
    const point = `POINT (${location.longitude} ${location.latitude})`;
    const query = new URLSearchParams({
    ...
    return {
        ...
        zoneDescription: firstText(record.description, record.major, record.generalize) || "Calgary land-use district",
        sourceUrl: "https://data.calgary.ca/d/qe6k-p9nh",
        geocodedAddress: location.displayName,
        rawData: record,
    };
}
async function lookupVancouver(location) {
    const query = new URLSearchParams({
    ...
    const response = await fetch(`https://opendata.vancouver.ca/api/explore/v2.1/catalog/datasets/zoning-districts-and-labels/records?${query}`, { headers: { Accept: "application/json" }, signal: AbortSignal.timeout(8000), cache: "no-store" });
    if (!response.ok)
        return null;
    ...
    return {
        ...
        zoneDescription: firstText(record.zoning_classification, record.zoning_category) || "Vancouver zoning district",
        sourceUrl: "https://opendata.vancouver.ca/explore/dataset/zoning-districts-and-labels/",
        geocodedAddress: location.displayName,
        rawData: record,
    };
}
async function lookupSurrey(location) {
    const source = "https://services5.arcgis.com/YRpe0VKTJytZSSIB/arcgis/rest/services/Zoning%20%20Boundaries/FeatureServer/0";
    const record = await queryArcGisPoint(source, location, "OBJECTID,ZONING_TEXT,ZONING,BY_LAW_01,BY_LAW_02,BY_LAW_03,BY_LAW_04,LOT_TYPE,WEBLINK");
    if (!record)
        return null;
    return {
...
async function lookupCourtenay(location) {
    const source = "https://services3.arcgis.com/PwS5hVLYsEN2U36s/arcgis/rest/services/Zoning/FeatureServer/0";
    const record = await queryArcGisPoint(source, location, "OBJECTID,Zone,Description,Comments,details,Zonning_byl,last_edited_date");
    if (!record)
        return null;
    return {
...
async function lookupToronto(location) {
    const source = "https://gis.toronto.ca/arcgis/rest/services/cot_geospatial11/FeatureServer/3";
    const record = await queryArcGisPoint(source, location, "OBJECTID,ZN_ZONE,ZN_STRING,ZN_HOLDING,ZN_EXCPTN,ZN_EXCPTN_NO,ZBL_CHAPTER,ZBL_SECTION,ZBL_EXCPTN,ZN_FRONTAGE,ZN_UNIT_COUNT,ZN_FSI_DENSITY,ZN_COVERAGE,FSI_TOTAL");
    if (!record)
        return null;
    return {
        ...
        sourceUrl: "https://www.toronto.ca/city-government/planning-development/zoning-by-law-preliminary-zoning-reviews/zoning-by-law-569-2013-2/",
        geocodedAddress: location.displayName,
        rawData: record,
    };
}
async function lookupSeattle(location) {
    const source = "https://services.arcgis.com/ZOyb2t4B0UYuYNYH/arcgis/rest/services/Current_Land_Use_Zoning_Detail_2/FeatureServer/0";
    const record = await queryArcGisPoint(source, location, "OBJECTID,ZONING,BASE_ZONE,ZONING_DESC,CLASS_DESC,CATEGORY_DESC,DETAIL_DESC,OVERLAY,HISTORIC,SHORELINE,LIGHTRAIL,EFFECTIVE,CHAPTER_LINK");
    if (!record)
        return null;
    return {
        ...
        zoneDescription: firstText(record.ZONING_DESC, record.DETAIL_DESC, record.CATEGORY_DESC) || "Seattle zoning district",
...
async function lookupPortland(location) {
    const source = "https://www.portlandmaps.com/od/rest/services/COP_OpenData_ZoningCode/MapServer/16";
    const record = await queryArcGisPoint(source, location, "OBJECTID,ZONE,ZONE_DESC,OVRLY,OVRLY_DESC,PLDIST,PLDIST_DESC,HIST,HIST_DESC,CONSV,CONSV_DESC,UNINC");
    if (!record)
        return null;
    return {
...
async function queryArcGisPoint(layerUrl, location, outFields) {
    const query = new URLSearchParams({
        f: "json",
        where: "1=1",
