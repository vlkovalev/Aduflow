...
function validateLead(body: Partial<CreateLeadInput>) {
  ...
  return {
    ...
    zoningSource: String(body.zoningSource ?? ""),
    zoningZone: String(body.zoningZone ?? ""),
    zoningDescription: String(body.zoningDescription ?? ""),
    zoningRaw: (body.zoningRaw ?? null) as Record<string, unknown> | null,
    zoningLookupStatus: String(body.zoningLookupStatus ?? "manual"),
    zoningCheckedAt: String(body.zoningCheckedAt ?? ""),
    aduPermitted: typeof body.aduPermitted === "boolean" ? body.aduPermitted : null,
    setbackFront: String(body.setbackFront ?? ""),
    setbackSide: String(body.setbackSide ?? ""),
    setbackRear: String(body.setbackRear ?? ""),
